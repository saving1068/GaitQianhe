<template>
	<div style="height:100vh;">
		...
		<div class="domestic" v-if="index == 0" style="height:80%;">
	...
	export default {
		...
		methods: {
			...
			goBack() {
				let obj = {}
				if(this.$route.query.activeIndex == 0) {
					obj = { ...this.$store.state.Home.made
					}
				} else {
					obj = { ...this.$store.state.Home.teamMade
					}
				}

				if(this.$route.query.type == "from") {
					obj.deparPlace = this.placeList[0];
					obj.fromId = this.siteIds[0]

				}
				if(this.$route.query.type == "to") {
					obj.destination = this.placeList.join(",");
					obj.toId = this.siteIds.join(",");

				}
				if(this.$route.query.activeIndex == 0) {
					this.$store.dispatch("changeMadeObj", obj)
				} else {
					this.$store.dispatch("changeTeamMadeObj", obj)
				}
