...
<template>
	<div class="affirmIndent">
		...
		<div class="numberBox cf">
			{{ tripManType == 1 ? '成人' : '儿童' }}
			<span class="number">¥<b v-text="buyAmount">1999</b></span>
			<span class="fr">剩余数量{{ surplusNum }}件</span>
		...
		<!--<section class="trip_text">
			...
			<form class="cf">
				<p class="fl"><span>*</span>姓名</p>
				<input type="text" placeholder="输入姓名" class="input_mag" v-model="name2"/>
			</form>
			<form class="cf">
				<p class="fl"><span>*</span>手机</p>
				<input type="text" placeholder="输入手机" class="input_mag" v-model="mobile2"/>
			</form>
			<form class="cf">
				<p class="fl"><span>*</span>身份证号</p>
				<input type="text" placeholder="输入身份证号码" v-model="Id2"/>
			</form>
			<form class="cf">
				<p class="fl"><span>*</span>其他信息</p>
				<input type="text" placeholder="若订单需要护照等信息请填写于此" v-model="txt2"/>
			</form>
		...
		<section class="discounts_text">
			...
			<div class="cf" @click="goChooseDiscount2(2,go.dateId,go.orgId,num,tripManType)">
				优惠券
		...
		<footer class="footer">
			...
			<p class="fl" style="text-decoration: none;color: #f6323e;">¥<b v-text="reductionAmounting">4999</b></p>
...
export default {
	data() {
		return {
			...
			couponId:Number(this.$route.query.couponId), //优惠券Id必传；
			spuId:Number(this.$route.query.spuId),  //产品id；
			grspuId:this.$route.query.grspuId,  //公告拼团点击的那条信息spuId；
			eq:this.$route.query.eq, //公告拼团点击的那条信息的下标,也是 groupList 数组里的下标；
			tripManType:Number(this.$route.query.tripManType),
			...
			isAcc:false,
			tAmount:this.$route.query.tAmount,  //从优惠券选中跳转过来的优惠券价格;
			...
			cash:null,
			isReturn:null,
			ID:null, //groupId,拼团用的id;
			skuId:null,
			shangciTime:false,
			surplusNum:null, //还剩多少参与拼团名额；
			
		}
	},
	created() {
		console.log(this.$route.query,'this.$route.query')
		this.GoodsDetails(); //商品详情接口；

		if(window.sessionStorage.getItem('num')) {
			this.num = window.sessionStorage.getItem('num')
		};

		this.tripMans = [...this.$store.state.Shop.tripMansing];
...
//							})
		...
		GoodsDetails(){
			...
			this.$Api.Shop.GoodsDetails(GDetObj).then( res => {
				...
				this.spec = res.q.goods.groupLists[this.eq].spec;
				this.placeDate = res.q.goods.groupLists[this.eq].placeDate;
				this.surplusNum = res.q.goods.groupLists[this.eq].surplusNum;
				this.ID = res.q.goods.groupLists[this.eq].id;
				this.skuId = res.q.goods.groupLists[this.eq].skuId;
		...
		handleReduce(){ //减少；
  			...
  			if (this.num == 1) { //数量不能小于1；
  				...
  				this.num--;
  				//this.groupLists[this.eq].surplusNum++;
  				this.tripMans.pop();
	  	...
	  	handleAdd(){ //增加；
	  		//判断增加按钮,剩余数量为0就不能加了；
	  		if (this.groupLists[this.eq].surplusNum == 0) {
	  			return
	  		}else if(this.num >= this.groupLists[this.eq].surplusNum){
	  			Toast('亲，已到达最大数量');
			...
			if(this.numOnoff){ //开一次的；
				if(window.sessionStorage.getItem('checkedGroup')){
					...
					Object.keys(obj).forEach(val=> {
						console.log('val',val,'this.dateId',this.dateId)
...
//						}
				...
				this.$Api.Shop.OrderSubmit(OrdObj).then( res => {
					...
					this.buyAmount = res.q.goods[0].buyAmount; //购买金额（商品的单价）；
		...
		OrderSubmit(){
			...
			if(!this.shangciTime || (startTime-this.shangciTime)>1500) {
				...
				//						],
							...
							console.log(OrdObj.couponId,'couponId参与拼团的优惠券id')
	...
	//							else if(res.q.s == 1605){ //不能参与自己拼的团(因为是旅游产品所以不限制用户)；
	//								Toast('不能参与自己拼的团!');
	//							}
							});
						}else{
		...
		click_yes(){
			...
			//console.log(location.href,'location.hreflocation.hreflocation.href1111')
			setTimeout( () => {
		...
		PayOkSubmiting(obj){
			this.$Api.Shop.PayOkSubmit(obj).then( res => {
				...
				this.surplusNum = res.q.list[0].surplusNum; //还剩多少参与拼团名额；
				
				//判断还剩最后一个参与拼团名额购买成功后，变成还剩0名额，没参与拼团名额了就跳我的订单页面；
				if (this.surplusNum == 0) { //没参与拼团名额了；
					this.$router.push({
						name: "MyIndent",
						query: {
		...
		goChooseDiscount2(a,dateId,orgId,num,tripManType){
			...
			if (this.catusecoupons.length==0) { //无；
				...
				console.log(a,dateId,orgId,num,tripManType,'dateId,orgId,num')
				this.$router.push({
	...
	beforeRouteLeave(to,from,next){ //离开页面时把this.tripMansing保存到vuex里的state全局变量；
		this.$store.commit('changeTripMansing',this.tripMans)
