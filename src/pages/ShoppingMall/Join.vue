	...
	<div class="joinShoppingCar">
		...
		<section>
			...
			<footer>
				...
				<div class="cf">
					<ul class="days">
						<li @click.stop="pick($event,day,days)" v-for="(day,index) in days" :class="{active_span: day.dateFlog == true}" ref="lisId">	<!--&& chosen == tripManType-->
...
//				this.shangciTime=startTime;
				...
				if (this.btnTxt == 2) { //立即购买;
					...
					//console.log(this.$refs.liId,'iIdiIdiIdiId2')
...
//							}
//						}
						if (this.tripManType == 2 && item.childrenGroupPrice == undefined) { //点击儿童时，且没有儿童活动价；
							item.priceFlog = false;

							console.log('没有儿童活动价')
						}else if(this.tripManType == 1 && item.groupPrice != undefined){ //是点击成人时，且有成人活动价；
							item.priceFlog = true;
...
//							}
//						}
						if (this.tripManType == 2 && item.childrenPrice == undefined) { //点击儿童时，且没有儿童正常价；
							item.priceFlog = false;
							console.log('没有儿童正常价')
						}else if (this.tripManType == 1 && item.price != undefined) { //点击成人时，且有成人正常价；
							item.priceFlog = true;
							console.log('有成人正常价')
						}
					}
				...
				if (this.dateId !== null) {
					this.$router.push({
						...
						query: {
							...
							btnTxt:this.btnTxt,
							couponId:0, //优惠券Id必传；
//							onoff:true  //用于控制这个AffirmIndent2页面vuex变量的开关；
						},
					})
				} else{
					Toast('亲，请选择日期！');
