<template>
	<div class="warp">
		<div class="appen_warp">
			<div class="appen_img">
				...
				<div class="appen_miuse" @click="appendMiuse">
					<p></p>
					<div class="miuseName" v-if="this.$store.state.sendTravels.miuseName">
						<span>{{musicName}}</span>
					</div>
					<font v-else>添加音乐</font>
					<audio v-show="this.$store.state.Travels.miuseName" id="a1" :src="musicPath"></audio>
				</div>
				<div class="stop_miuse" @click="playOrStop" v-if="this.$store.state.sendTravels.miuseName" >
					<img v-if="!miusePlaying" src="../../assets/game/common_btn_suspend@2x.png"/>
					<img  v-else style="width: 40%;" src="../../assets/game/common_btn_play@2x.png"/>
				</div>
			</div>
			<div class="title">
				<div>
					<van-cell-group>
						<van-field v-model="message" type="textarea" placeholder="请输入游记标题" :autosize="minHeight" maxlength="50"@focus="onFocus" @blur="onBlur" />
					</van-cell-group>
				</div>
				<div class="title_val">
					<font v-if="message">可输入{{50-message.length}}字</font>
					<font v-else>可输入50字</font>
				</div>
			</div>
			...
			<div class="location" @click="appendPlace">
				<span>
					<p></p>
					<font v-if="sites">{{sites}}</font>
					<font v-else>添加游记地点</font>
				</span>
			</div>
			<div class="appen_actial" v-if="this.$store.state.sendTravels.send.length == 0" @click="openBtn" v-show="!btn_show">
				<p></p>
			</div>
			<div class="item_btn" v-show="btn_show">
		...
		<div class="item" v-for="(v,index) in this.$store.state.sendTravels.send">
			...
			<div  v-if="v.protection == 2">
				<a target="_blank" :href="v.content" style="width: 100%;">
					<div class="item_img">
			...
			<div v-if="v.protection == 5">
				<!--<div id="id_test_video" style="width:100%; height:100%;"></div>-->
				<div  class="item_img">
					<video x5-video-player-type="h5" x5-video-player-fullscreen="true" x-webkit-airplay="allow" webkit-playsinline preload="auto" playsinline v-if="v.path" :src="v.path" style="width: 100%;height: 100%;object-fit: fill"  controls="controls"></video>
					<video x5-video-player-type="h5" x5-video-player-fullscreen="true" x-webkit-airplay="allow" webkit-playsinline preload="auto" playsinline v-else  style="width: 100%;height: 100%;object-fit: fill" :src="v.content"  controls="controls"></video>
				</div>
				<div class="item_location">
					<div class="item_location_font">
						<div class="Spot" @click="goToSpot(index)" v-if="v.location">
		...
		<div class="appen_actial" v-if="this.$store.state.sendTravels.send.length > 0&&!lastBtn" @click="openLastBtn">
				<p></p>
		</div>
		<div class="item_btn" v-show="lastBtn">
			<div class="btn_item" v-for="(v,i) in btn_item" @click="btn_choise(v,i,index,'last')">
				<p><img :src="v.img" /></p>
				<font>{{v.name}}</font>
				<input type="file" v-if="i==1" @change="lastVideo" />
			</div>
		</div>
		<div :style="[{'height':show?'0':'4.25rem'}]">

		</div>
		<div class="bottom_btn" :style="[{'position':show?'fixed':'static'}]" v-if="!showkeep">
		...
		<div class="bottom_btn" :style="[{'position':show?'fixed':'static'}]" v-else>
			<div class="btn_item">
				<p class="left_p"></p>
				<font>保存</font>
			</div>
		...
		<Cancel v-show="ifCancel" @delete='leave' @cancel="notleave"></Cancel>
...
<script>
	...
	export default {
		...
		created() {
			console.log(this.$store.state.sendTravels,"sendTravels")
			if(this.$store.state.sendTravels.coverImg){
				this.imagePath = this.$store.state.sendTravels.coverImg
			}
			if(this.$store.state.sendTravels.cityName){
				this.sites = this.$store.state.sendTravels.cityName
			}
			if(this.$store.state.sendTravels.miuseName){
				this.musicName = this.$store.state.sendTravels.miuseName;
				this.musicPath = this.$store.state.sendTravels.miusePath;
			}
...
//                  	that.play()
			...
			coverImage(e){
				...
				oFReader.onload = function(oFREvent) {
					...
					that.imagePath= imgData
					that.$store.state.sendTravels.coverImg = imgData//本地图片路径，
//					console.log(this.$store.state.sendTravels.coverImg,"封面图片本地路径")
				}
			},
			lastVideo(e){
				var that = this
				...
				if(fileSize <=4&&fileSize>0){
					...
					this.$Api.Common.GetExternalUploadObject(obj).then((res) => {
						...
						if(res.q.s == 0) {
							...
							//上传图片到腾讯云
							this.$upLoadToCos(res.q.cos.url, e.target.files[0])
							this.getExternalFilesSubmit(urlObj, obj.type,"item","last")
...
//				console.log(e)
				...
				if(fileSize <=4&&fileSize>0){
					...
					this.$Api.Common.GetExternalUploadObject(obj).then((res) => {
						...
						if(res.q.s == 0) {
							...
							//上传图片到腾讯云
							this.$upLoadToCos(res.q.cos.url, e.target.files[0])
							this.getExternalFilesSubmit(urlObj, obj.type,"item")
	...
	//				console.log(that.$store.state.TravelsArticle)
			...
			getExternalFilesSubmit(urlObj, type,font,last) {
//				console.log(urlObj)
				let fileInfos = []
				...
				this.$Api.Common.ExternalFilesSubmit(obj).then((res) => {
					...
					if(last){
						if(font == "cover"){
							this.$store.state.sendTravels.imgId	= res.q.ids[0] //封面图Id
	//						this.$store.state.sendTravels.coverImg = res.q.files[0].path //封面图本地路径
						}else{
							if(type == 1) {
									...
									var obj_a = {
										...
										fileId:this.itmeImgId//图片Id
									}
									this.$store.state.sendTravels.send.push(obj_a)
								}else{
									this.itmeImgId = res.q.ids[0]
									...
									var obj_a = {
										...
										fileId:this.itmeImgId//视频Id
									}
									this.$store.state.sendTravels.send.push(obj_a)
	//								this.$nextTick(
	//									var player = new TcPlayer('id_test_video', {
	//										"m3u8": that.sendVideoPath, //请替换成实际可用的播放地址
	...
	//						console.log(obj_a)
	//						this.$store.state.sendTravels.send.splice(this.itemNum,0,obj_a)
	//						console.log(this.$store.state.sendTravels.send)
	...
	//						console.log(this.$store.state.sendTravels.send)
			...
			WbSubmit() {
				var list = this.$store.state.sendTravels.send;
				let spotIds =[];
...
//							id:this.$route.query.id,
							imageId:this.$store.state.sendTravels.imgId,
							musicId:this.$store.state.sendTravels.miuseId,
							destinationIds:this.$store.state.sendTravels.siteIds,
							title:this.$store.state.sendTravels.title,
							spotIds:spotIds.length>0?spotIds.join(","):0,
							contents:list,
						}
						if(!this.$store.state.sendTravels.title){
...
//							console.log(obj.destinationIds)
//							console.log(obj.contents.length)
//							console.log(obj.imageId)
							this.$Api.UserPage.WbSubmit(obj).then((res) => {
								...
								if(this.ifKeep == 1){
									if(this.showkeep){
										if(res.q.s == 0){
											this.$store.state.sendTravels = {};
											this.$store.state.sendTravels.send = [];
											this.message='';
											this.$router.go(-1);
...
//						id:this.$route.query.id
			...
			sureDelete() {
				console.log("确认删除")
				this.$store.state.sendTravels.send.splice(this.deleteNum,1)
				console.log(this.$store.state.sendTravels.send)
				this.ifDelete = false
			},
			cancelDelete() {
...
//				console.log(this.imagePath)
			...
			appenList(){
				if(this.$store.state.sendTravels.send.length == 0){
					console.log("没有变化")
					return this.$store.state.sendTravels.send
				}else{
					return this.$store.state.sendTravels.send
				}
			},
...
//			console.log(from)
			next((vm)=>{ //参数vm就是当前组件的实例。
				console.log(vm.$store.state.sendTravels.title)
					vm.imagePath = vm.$store.state.sendTravels.coverImg;
					vm.message = vm.$store.state.sendTravels.title;

            })
		},
		beforeRouteLeave(to,from,next){
			this.$store.state.sendTravels.title = this.message
				next()
